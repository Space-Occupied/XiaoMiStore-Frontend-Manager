    ...
    <div class="main" ref="containerRef">
        ...
        <div class="data">
            <el-table :data="tableData" border style="width: 100%" :height="tableHeight" @row-click="handleRow"
                ...
                <el-table-column prop="name" label="类别名" width="180" />
                <el-table-column prop="pid" label="父类别ID" width="100" />
                <el-table-column prop="isDel" label="是否已删除" width="100" />
                    ...
                    <el-row :gutter="20">
                        <el-col :span="12">
                            <el-form-item label="类别名：" label-width="100">
                                <el-input v-model="productCategoryInfo.name" />
                            </el-form-item>
                        </el-col>
                        <el-col :span="12">
                            <el-form-item label="父类别ID：" label-width="100">
                                <el-input v-model="productCategoryInfo.pid" />
                            </el-form-item>
                        </el-col>
                    </el-row>
                    <el-row :gutter="20">
                        <el-col :span="24">
                            <el-form-item label="是否已删除：" label-width="100">
                                <el-input v-model="productCategoryInfo.isDel" />
                            </el-form-item>
                        </el-col>
                    </el-row>
                </el-form>
...
import {
    ...
    addProductCategory as apiAddProductCategory, deleteByIds as apiDeleteByIds, updateProductCategory as apiUpdateProductCategory
...
function handleRow(row, column, event) {
    ...
    if (prepareEdit === true) {
        prepareEdit = false;
        changingId = row.id;
        productCategoryInfo.value = Object.assign({}, row);
        dialogTitle.value = "修改商品类别";
        // console.log(row.id);
...
async function onDialogSubmit() {
    let resp = null;
    if (changingId == null) {
        productCategoryInfo.value.id = null;
        resp = await apiAddProductCategory(productCategoryInfo.value);
        if (resp.success) {
            ElMessage({
                type: 'success',
                message: '添加成功',
        ...
        productCategoryInfo.value.id = changingId;
        resp = await apiUpdateProductCategory(productCategoryInfo.value);
        if (resp.success) {
            ElMessage({
                type: 'success',
                message: '修改成功',
