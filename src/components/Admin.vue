...
import {
    ...
    addAdmin as apiAddAdmin, deleteByIds as apiDeleteByIds, updateAdmin as apiUpdateAdmin
} from '@/api/AdminApi';
...
function handleRow(row, column, event) {
    ...
    if (prepareEdit === true) {
        prepareEdit = false;
        changingId = row.id;
        adminInfo.value = Object.assign({}, row);
...
async function onDialogSubmit() {
    ...
    if (changingId == null) {
        ...
        resp = await apiAddAdmin(adminInfo.value);
        if (resp.success) {
            ElMessage({
                type: 'success',
                message: '添加成功',
        ...
        adminInfo.value.id = changingId;
        resp = await apiUpdateAdmin(adminInfo.value);
        if (resp.success) {
            ElMessage({
                type: 'success',
                message: '修改成功',
