    ...
    <div class="main" ref="containerRef">
        ...
        <div class="data">
            <el-table :data="tableData" border style="width: 100%" :height="tableHeight" @row-click="handleRow"
                ...
                <el-table-column prop="actionId" label="购买记录ID" width="200" />
                <el-table-column prop="choiceId" label="选项ID" width="200" />
        ...
        <div class="form">
            <el-dialog v-model="dialogVisible" :title="dialogTitle" width="800" draggable @close="clearDialogForm">
                <el-form>
                    <el-row :gutter="20">
                        <el-col :span="12">
                            <el-form-item label="购买记录ID：" label-width="100">
                                <el-input v-model="actionChoiceInfo.actionId" />
                            </el-form-item>
                        </el-col>
                        <el-col :span="12">
                            <el-form-item label="选项ID：" label-width="100">
                                <el-input v-model="actionChoiceInfo.choiceId" />
                            </el-form-item>
                        </el-col>
                    </el-row>
                </el-form>
...
import {
    ...
    addActionChoice as apiAddActionChoice, deleteByIds as apiDeleteByIds, updateActionChoice as apiUpdateActionChoice
} from '@/api/ActionChoiceApi';
...
function handleRow(row, column, event) {
    ...
    if (prepareEdit === true) {
        prepareEdit = false;
        changingId = row.id;
        actionChoiceInfo.value = Object.assign({}, row);
        dialogTitle.value = "修改记录";
        // console.log(row.id);
...
async function onDialogSubmit() {
    let resp = null;
    if (changingId == null) {
        actionChoiceInfo.value.id = null;
        resp = await apiAddActionChoice(actionChoiceInfo.value);
        if (resp.success) {
            ElMessage({
                type: 'success',
                message: '添加成功',
        ...
        actionChoiceInfo.value.id = changingId;
        resp = await apiUpdateActionChoice(actionChoiceInfo.value);
        if (resp.success) {
            ElMessage({
                type: 'success',
                message: '修改成功',
