    ...
    <div class="main" ref="containerRef">
        ...
        <div class="data">
            <el-table :data="tableData" border style="width: 100%" :height="tableHeight" @row-click="handleRow"
                ...
                <el-table-column prop="fullName" label="商品全名" width="200" />
        ...
        <div class="form">
            <el-dialog v-model="dialogVisible" :title="dialogTitle" width="800" draggable @close="clearDialogForm">
                <el-form>
                    <el-row :gutter="20">
                        <el-col :span="12">
                            <el-form-item label="用户名ID：" label-width="100">
                                <el-input v-model="cartInfo.userId" />
                            </el-form-item>
                        </el-col>
                        <el-col :span="12">
                            <el-form-item label="商品ID：" label-width="100">
                                <el-input v-model="cartInfo.productId" />
                            </el-form-item>
                        </el-col>
                    </el-row>
                    <el-row :gutter="20">
                        <el-col :span="12">
                            <el-form-item label="数量：" label-width="100">
                                <el-input v-model="cartInfo.quantity" />
                            </el-form-item>
                        </el-col>
                        <el-col :span="12">
                            <el-form-item label="商品全名：" label-width="100">
                                <el-input v-model="cartInfo.fullName" />
                            </el-form-item>
                        </el-col>
                    </el-row>
                </el-form>
...
import {
    ...
    addCart as apiAddCart, deleteByIds as apiDeleteByIds, updateCart as apiUpdateCart
} from '@/api/CartApi';
...
function handleRow(row, column, event) {
    ...
    if (prepareEdit === true) {
        prepareEdit = false;
        changingId = row.id;
        cartInfo.value = Object.assign({}, row);
...
async function onDialogSubmit() {
    let resp = null;
    if (changingId == null) {
        cartInfo.value.id = null;
        resp = await apiAddCart(cartInfo.value);
        if (resp.success) {
            ElMessage({
                type: 'success',
                message: '添加成功',
        ...
        cartInfo.value.id = changingId;
        resp = await apiUpdateCart(cartInfo.value);
        if (resp.success) {
            ElMessage({
                type: 'success',
                message: '修改成功',
