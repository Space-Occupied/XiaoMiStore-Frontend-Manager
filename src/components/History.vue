    ...
    <div class="main" ref="containerRef">
        ...
        <div class="data">
            <el-table :data="tableData" border style="width: 100%" :height="tableHeight" @row-click="handleRow"
                ...
                <el-table-column prop="viewTime" label="浏览时间" width="200" />
                    ...
                    <el-row :gutter="20">
                        <el-col :span="12">
                            <el-form-item label="用户ID：" label-width="100">
                                <el-input v-model="historyInfo.userId" />
                            </el-form-item>
                        </el-col>
                        <el-col :span="12">
                            <el-form-item label="商品ID：" label-width="100">
                                <el-input v-model="historyInfo.productId" />
                            </el-form-item>
                        </el-col>
                    </el-row>
                    <el-row :gutter="20">
                        <el-col :span="24">
                            <el-form-item label="浏览时间：" label-width="100">
                                <el-input v-model="historyInfo.viewTime" />
                            </el-form-item>
                        </el-col>
                    </el-row>
                </el-form>
...
import {
    ...
    addHistory as apiAddHistory, deleteByIds as apiDeleteByIds, updateHistory as apiUpdateHistory
} from '@/api/HistoryApi';
...
function handleRow(row, column, event) {
    ...
    if (prepareEdit === true) {
        prepareEdit = false;
        changingId = row.id;
        historyInfo.value = Object.assign({}, row);
        dialogTitle.value = "修改浏览记录";
        // console.log(row.id);
...
async function onDialogSubmit() {
    let resp = null;
    if (changingId == null) {
        historyInfo.value.id = null;
        resp = await apiAddHistory(historyInfo.value);
        if (resp.success) {
            ElMessage({
                type: 'success',
                message: '添加成功',
        ...
        historyInfo.value.id = changingId;
        resp = await apiUpdateHistory(historyInfo.value);
        if (resp.success) {
            ElMessage({
                type: 'success',
                message: '修改成功',
