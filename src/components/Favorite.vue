                    ...
                    <el-row :gutter="20">
                        <el-col :span="12">
                            <el-form-item label="用户ID：" label-width="100">
                                <el-input v-model="favoriteInfo.userId" />
                            </el-form-item>
                        </el-col>
                        <el-col :span="12">
                            <el-form-item label="商品ID：" label-width="100">
                                <el-input v-model="favoriteInfo.productId" />
                            </el-form-item>
                        </el-col>
                    </el-row>
                    <el-row :gutter="20">
                        <el-col :span="24">
                            <el-form-item label="添加时间：" label-width="100">
                                <el-input v-model="favoriteInfo.addTime" />
                            </el-form-item>
                        </el-col>
                    </el-row>
                </el-form>
...
import {
    ...
    addFavorite as apiAddFavorite, deleteByIds as apiDeleteByIds, updateFavorite as apiUpdateFavorite
...
function handleRow(row, column, event) {
    ...
    if (prepareEdit === true) {
        prepareEdit = false;
        changingId = row.id;
        favoriteInfo.value = Object.assign({}, row);
        dialogTitle.value = "修改收藏";
        // console.log(row.id);
...
async function onDialogSubmit() {
    let resp = null;
    if (changingId == null) {
        favoriteInfo.value.id = null;
        resp = await apiAddFavorite(favoriteInfo.value);
        if (resp.success) {
            ElMessage({
                type: 'success',
                message: '添加成功',
        ...
        favoriteInfo.value.id = changingId;
        resp = await apiUpdateFavorite(favoriteInfo.value);
        if (resp.success) {
            ElMessage({
                type: 'success',
                message: '修改成功',
