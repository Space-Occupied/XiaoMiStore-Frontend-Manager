    ...
    <div class="main" ref="containerRef">
        ...
        <div class="data">
            <el-table :data="tableData" border style="width: 100%" :height="tableHeight" @row-click="handleRow"
                ...
                <el-table-column prop="name" label="选项名" width="120" />
                <el-table-column prop="price" label="价格" width="120" />
                <el-table-column prop="oldPrice" label="原价" width="120" />
                <el-table-column prop="optionId" label="选项组ID" width="200" />
        ...
        <div class="form">
            <el-dialog v-model="dialogVisible" :title="dialogTitle" width="800" draggable @close="clearDialogForm">
                <el-form>
                    <el-row :gutter="20">
                        <el-col :span="12">
                            <el-form-item label="选项组ID：" label-width="100">
                                <el-input v-model="choiceInfo.optionId" />
                            </el-form-item>
                        </el-col>
                        <el-col :span="12">
                            <el-form-item label="选项名：" label-width="100">
                                <el-input v-model="choiceInfo.name" />
                            </el-form-item>
                        </el-col>
                    </el-row>
                    <el-row :gutter="20">
                        <el-col :span="12">
                            <el-form-item label="价格：" label-width="100">
                                <el-input v-model="choiceInfo.price" />
                            </el-form-item>
                        </el-col>
                        <el-col :span="12">
                            <el-form-item label="原价：" label-width="100">
                                <el-input v-model="choiceInfo.oldPrice" />
                            </el-form-item>
                        </el-col>
                    </el-row>
                </el-form>
...
import {
    ...
    addChoice as apiAddChoice, deleteByIds as apiDeleteByIds, updateChoice as apiUpdateChoice
} from '@/api/ChoiceApi';
...
function handleRow(row, column, event) {
    ...
    if (prepareEdit === true) {
        prepareEdit = false;
        changingId = row.id;
        choiceInfo.value = Object.assign({}, row);
...
async function onDialogSubmit() {
    let resp = null;
    if (changingId == null) {
        choiceInfo.value.id = null;
        resp = await apiAddChoice(choiceInfo.value);
        if (resp.success) {
            ElMessage({
                type: 'success',
                message: '添加成功',
        ...
        choiceInfo.value.id = changingId;
        resp = await apiUpdateChoice(choiceInfo.value);
        if (resp.success) {
            ElMessage({
                type: 'success',
                message: '修改成功',
