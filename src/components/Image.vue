    ...
    <div class="main" ref="containerRef">
        ...
        <div class="data">
            <el-table :data="tableData" border style="width: 100%" :height="tableHeight" @row-click="handleRow"
                ...
                <el-table-column prop="url" label="图片路径" width="200" />
                    ...
                    <el-row :gutter="20">
                        <el-col :span="12">
                            <el-form-item label="类型：" label-width="100">
                                <el-input v-model="imageInfo.type" />
                            </el-form-item>
                        </el-col>
                        <el-col :span="12">
                            <el-form-item label="子ID：" label-width="100">
                                <el-input v-model="imageInfo.subId" />
                            </el-form-item>
                        </el-col>
                    </el-row>
                    <el-row :gutter="20">
                        <el-col :span="24">
                            <el-form-item label="图片路径：" label-width="100">
...
import {
    ...
    addImage as apiAddImage, deleteByIds as apiDeleteByIds, updateImage as apiUpdateImage
} from '@/api/ImageApi';
...
function handleRow(row, column, event) {
    ...
    if (prepareEdit === true) {
        prepareEdit = false;
        changingId = row.id;
        imageInfo.value = Object.assign({}, row);
...
async function onDialogSubmit() {
    let resp = null;
    if (changingId == null) {
        imageInfo.value.id = null;
        resp = await apiAddImage(imageInfo.value);
        if (resp.success) {
            ElMessage({
                type: 'success',
                message: '添加成功',
        ...
        imageInfo.value.id = changingId;
        resp = await apiUpdateImage(imageInfo.value);
        if (resp.success) {
            ElMessage({
                type: 'success',
                message: '修改成功',
