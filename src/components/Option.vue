    ...
    <div class="main" ref="containerRef">
        ...
        <div class="data">
            <el-table :data="tableData" border style="width: 100%" :height="tableHeight" @row-click="handleRow"
                ...
                <el-table-column prop="name" label="选项组名" width="200" />
        ...
        <div class="form">
            <el-dialog v-model="dialogVisible" :title="dialogTitle" width="800" draggable @close="clearDialogForm">
                <el-form>
                    <el-row :gutter="20">
                        <el-col :span="12">
                            <el-form-item label="选项组名：" label-width="100">
                                <el-input v-model="optionInfo.name" />
                            </el-form-item>
                        </el-col>
                        <el-col :span="12">
                            <el-form-item label="商品ID：" label-width="100">
                                <el-input v-model="optionInfo.productId" />
                            </el-form-item>
                        </el-col>
                    </el-row>
                </el-form>
...
import {
    ...
    addOption as apiAddOption, deleteByIds as apiDeleteByIds, updateOption as apiUpdateOption
} from '@/api/OptionApi';
...
function handleRow(row, column, event) {
    ...
    if (prepareEdit === true) {
        prepareEdit = false;
        changingId = row.id;
        optionInfo.value = Object.assign({}, row);
        dialogTitle.value = "修改选项组";
        // console.log(row.id);
...
async function onDialogSubmit() {
    let resp = null;
    if (changingId == null) {
        optionInfo.value.id = null;
        resp = await apiAddOption(optionInfo.value);
        if (resp.success) {
            ElMessage({
                type: 'success',
                message: '添加成功',
        ...
        optionInfo.value.id = changingId;
        resp = await apiUpdateOption(optionInfo.value);
        if (resp.success) {
            ElMessage({
                type: 'success',
                message: '修改成功',
